 ...
 * returns 0 on success, 1 on lseek error, 2 on file error
 */
int fseek(FILE *fp, long offset, int whence)
{
    ...
    if (fp->flag & _ERR)
        return 2;
    /* clear the EOF flag, in case we're moving back the position */
    fp->flag &= ~_EOF;
    if (lseek(fp->fd, offset, whence) < 0) /* move the file's cursor */
        return 1;
    /* and reset the internal buffer cursor: this way, the next call
     * to getc or putc will fill/flush the buffer */
    fp->cnt = 0;
    fp->ncpos = fp->base;
    return 0;
}
